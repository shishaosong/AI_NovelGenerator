...
def build_ai_config_tab(self):
    def on_interface_format_changed(new_value):
        self.interface_format_var.set(self.loaded_config["llm_configs"][new_value].get("interface_format", "OpenAI"))
    ...
    def add_new_config():
        ...
        if new_config_name in self.loaded_config.get("llm_configs", {}):
            messagebox.showerror("错误", "配置名称已存在！")
            return
        self.loaded_config["llm_configs"][new_config_name] = {
            "api_key": "",
            "base_url": "",
            "model_name": "",
            "temperature": 0.7,
    ...
    def delete_config():
        selected_config = self.interface_format_var.get()
    ...
    #接口配置
    create_label_with_help(self, parent=self.ai_config_tab, label_text="接口配置", tooltip_key="interface_config", row= 0, column=0, font=("Microsoft YaHei", 12))
    interface_config_dropdown = ctk.CTkOptionMenu(self.ai_config_tab, values=list(self.loaded_config.get("llm_configs", {}).keys()), variable=self.interface_config_var, command=on_interface_format_changed, font=("Microsoft YaHei", 12))

    interface_config_dropdown.grid(row=0, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    #新增删除
    add_btn = ctk.CTkButton(self.ai_config_tab, text="新增配置", command=add_new_config, font=("Microsoft YaHei", 12))
    add_btn.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

    delete_btn = ctk.CTkButton(self.ai_config_tab, text="删除配置", command=delete_config, font=("Microsoft YaHei", 12))
    delete_btn.grid(row=1, column=2, padx=5, pady=5, sticky="ew")



    # 1) API Key
    create_label_with_help(self, parent=self.ai_config_tab, label_text="LLM API Key:", tooltip_key="api_key", row=2, column=0, font=("Microsoft YaHei", 12))
    api_key_entry = ctk.CTkEntry(self.ai_config_tab, textvariable=self.api_key_var, font=("Microsoft YaHei", 12),show="*")
    api_key_entry.grid(row=2, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 2) Base URL
    create_label_with_help(self, parent=self.ai_config_tab, label_text="LLM Base URL:", tooltip_key="base_url", row=3, column=0, font=("Microsoft YaHei", 12))
    base_url_entry = ctk.CTkEntry(self.ai_config_tab, textvariable=self.base_url_var, font=("Microsoft YaHei", 12))
    base_url_entry.grid(row=3, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 3) 接口格式
    create_label_with_help(self, parent=self.ai_config_tab, label_text="LLM 接口格式:", tooltip_key="interface_format", row=4, column=0, font=("Microsoft YaHei", 12))
    # 在接口选项列表中添加 "Grok"
    ...
    interface_dropdown.grid(row=4, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 4) Model Name
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Model Name:", tooltip_key="model_name", row=5, column=0, font=("Microsoft YaHei", 12))
    model_name_entry = ctk.CTkEntry(self.ai_config_tab, textvariable=self.model_name_var, font=("Microsoft YaHei", 12))
    model_name_entry.grid(row=5, column=1, padx=5, pady=5, columnspan=2, sticky="nsew")

    # 5) Temperature
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Temperature:", tooltip_key="temperature", row=6, column=0, font=("Microsoft YaHei", 12))
    ...
    temp_scale.grid(row=6, column=1, padx=5, pady=5, sticky="we")
    self.temp_value_label = ctk.CTkLabel(self.ai_config_tab, text=f"{self.temperature_var.get():.2f}", font=("Microsoft YaHei", 12))
    self.temp_value_label.grid(row=6, column=2, padx=5, pady=5, sticky="w")

    # 6) Max Tokens
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Max Tokens:", tooltip_key="max_tokens", row=7, column=0, font=("Microsoft YaHei", 12))
    ...
    max_tokens_slider.grid(row=7, column=1, padx=5, pady=5, sticky="we")
    self.max_tokens_value_label = ctk.CTkLabel(self.ai_config_tab, text=str(self.max_tokens_var.get()), font=("Microsoft YaHei", 12))
    self.max_tokens_value_label.grid(row=7, column=2, padx=5, pady=5, sticky="w")

    # 7) Timeout (sec)
    create_label_with_help(self, parent=self.ai_config_tab, label_text="Timeout (sec):", tooltip_key="timeout", row=8, column=0, font=("Microsoft YaHei", 12))
